...
`pca.m`を完了すると、`ex7_pca.m`スクリプトはサンプル・データセットでPCAを実行し、見つかった対応する主成分をプロットします（図5）。
...
`projectData.m`のコードを完成すると、`ex7_pca.m`は最初の次元に最初のサンプルを射影し、約1.481の値を表示します（U1の代わりに-U1を指定した場合は-1.481）。
...
あなたがすべきことは、`recoverData.m`を完了して、Zの各例を元の空間に射影し、リカバーした近似をX_recに返すことです。
...
`projectData`と`recoverData`の両方を完成すると、`ex7_pca.m`は射影と近似再構成の両方を実行し、射影がどのようにデータに影響するかを示します。
...
データセット`ex7faces.mat`には、32×32のグレースケールの顔画像のデータセットX（※3）が含まれています。
...
これらの主成分を、元のデータセットのピクセルに対応する32×32行列に再構成することによって、これらの主成分を可視化することができます。
...
`ex7_pca.m`では、データのおおよその回復が実行され、元の画像と射影された顔画像が並べて表示されます（図9）。
