...
## この演習に含まれるファイル
 ...
 - [\*] `computeCentroids.m` - 重心の平均の計算する（K-meansで使用）
 - [\*] `kMeansInitCentroids.m` - K-meansの重心を初期化する
...
最初に、K-meansアルゴリズムの仕組みを直感的に理解するために、2Dデータセットのサンプルを表示することから始めます。
その後、画像に現れる色の数をその画像で最も一般的な色の数に減らすため、画像圧縮にK-meansアルゴリズムを使用します。
演習のこのパートでは、`ex7.m`を使用します。

### 1.1. K-meansの実装

K-meansアルゴリズムは、類似のデータのサンプルを自動的にクラスタリングする手順です。
...
K-meansアルゴリズムは、いくつかの重心の平均の最終的な集合に常に収束します。
...
具体的には、設定したすべてのサンプル<img src="https://latex.codecogs.com/gif.latex?\inline&space;i" title="i" />に対して、以下をセットします。
...
与えられたすべての点の重心の割り当てについて、アルゴリズムの第2段階は、各重心に対して、それに割り当てられた点の平均を再計算します。
具体的には、すべての重心<img src="https://latex.codecogs.com/gif.latex?k" title="k" />に対して、

 ![式2](images/ex7/ex7-NF2.png)

ここで<img src="https://latex.codecogs.com/gif.latex?C_{k}" title="C_{k}" />は重心<img src="https://latex.codecogs.com/gif.latex?k" title="k" />に割り当てられたサンプルの集合です。
...
次のステップを実行すると、K-meansコードは、各繰り返しでアルゴリズムの進捗状況を可視化します。
<kbd>Enter</kbd>キーを何度か押して、K-meansアルゴリズムの各ステップが重心とクラスター割り当てをどのように変更するかを確認してください。
最後に、図は図1のように表示されます。
...
```
% ランダムに選択したサンプルで重心を初期化する

% サンプルのインデックスをランダムに並べ替える
randidx = randperm(size(X, 1));
% 最初のK個のサンプルを選択し、重心とする
centroids = X(randidx(1:K), :);
```

上記のコードは、最初に（`randperm`を使用して）サンプルのインデックスをランダムに置き換えます。
次に、インデックスをランダム置換した結果から最初の<img src="https://latex.codecogs.com/gif.latex?\inline&space;K" title="K" />個のサンプルを選択します。
...
画像（※2）の簡単な24ビットカラー表現では、各ピクセルは、赤、緑、青の輝度値を指定する3つの8ビット符号なし整数（0～255の範囲）で表現されます。
...
※2: この演習で使用している写真は、Frank Wouters氏のものであり、彼の許可を得て使用しています。
...
```
% 128x128カラー画像（bird_small.png）をロードする
A = imread('bird small.png');

% imreadを使用するには、画像パッケージをインストールする必要があります。
% 画像パッケージをインストールしていない場合は、代わりに次の行を変更する
% 必要があります。
%
%    load('bird small.mat'); % 画像を変数Aにロードする
```

これにより、三次元行列`A`の最初の2つのインデックスがピクセルの位置を識別し、最後のインデックスが赤、緑、青のいずれかを表します。
...
画像を表すために上位<img src="https://latex.codecogs.com/gif.latex?K&space;=&space;16" title="K = 16" />色を見つけた後、`findClosestCentroids`関数を使用して、各ピクセルの位置を最も近い重心に割り当てることができます。
これにより、各ピクセルの重心の割り当てを使用して、元の画像を表現することができます。
画像を記述するために必要なビット数が、大幅に削減されていることに注目してください。
...
図3に、得られた再構築の画像を示します。
結果として得られる画像にはオリジナルの特性のほとんどが残っていますが、いくつかの圧縮のアーティファクト（画像の乱れ）もあります。
...
最初に、2Dデータセットのサンプルを試して、PCAの仕組みを直感的に理解し、それを5000個の顔画像のより大きなデータセットに使用します。
提供されたスクリプト、`ex7_pca.m`は、演習の後半を実行するのに役立ちます。
...
<img src="https://latex.codecogs.com/gif.latex?\inline&space;\Sigma" title="\Sigma" />は<img src="https://latex.codecogs.com/gif.latex?\inline&space;n&space;\times&space;n" title="n \times n" />の行列であり、総和の演算子ではないことに注意してください。
...
射影されたデータを使用することで、入力の次元が少なくなるため、モデルをより早くトレーニングすることができます。
...
あなたがすべきことは、`recoverData.m`を完了して、`Z`の各サンプルを元の空間に射影し、復元した近似を`X_rec`に返すことです。
`recoverData.m`でコードを完成すると、`ex7_pca.m`は最初のサンプルの近似値を復元し、約`[-1.047 -1.047]`の値が表示されます。
...
演習のこのパートでは、顔画像でPCAを実行して、実際に次元削減のためにそれをどのように使用できるかを確認します。
...
`U`の各主成分（各行）は、長さ<img src="https://latex.codecogs.com/gif.latex?\inline&space;n" title="n" />のベクトルです（顔のデータセットの場合、<img src="https://latex.codecogs.com/gif.latex?\inline&space;n&space;=&space;1024" title="n = 1024" />）。
これらの主成分を、元のデータセットのピクセルに対応する32×32の行列に再構築することによって、これらの主成分を可視化することができます。
スクリプト`ex7_pca.m`は、最大の変動を描写する最初の36個の主成分を表示します（図8）。
必要に応じて、コードを変更してより多くの主成分を表示し、より詳細なキャプチャーの方法を確認することもできます。

#### 2.4.2. 次元の削減

これで、顔のデータセットの主成分を計算したので、これを使用して顔データセットの次元を減らすことができます。
...
これは、学習アルゴリズムを大幅にスピードアップするのに役立つ、データセットサイズの大幅な（10倍以上の）削減です。
たとえば、人の認識（顔の画像を与えて、人物の識別を予測する）を行うためにニューラル・ネットワークをトレーニングしていた場合、元のピクセルの代わりに100次元のピクセルの削減された入力を使用できます。
...
図の上でマウスをドラッグすると、このデータを3次元で回転して調べることができます。
