%% 機械学習オンラインクラス - 演習 4 ニューラル・ネットワークの学習
...
input_layer_size  = 400;  % 20x20の入力の数字画像
hidden_layer_size = 25;   % 25個の隠れユニット
num_labels = 10;          % 10個のラベル（1～10）
                          % （ラベル10に"0"をマッピングしていることに注意してください）

%% =========== パート 1: データのロードと可視化 =============
%  最初にデータセットをロードして可視化することから始めます。
...
%% ================ パート 5: シグモイド勾配  ================
%  ニューラル・ネットワークの実装を開始する前に、まずシグモイド関数の勾配を
%  実装します。sigmoidGradient.mファイルのコードを完成させる必要があります。
%  
%
...
%  これは "fminunc"と同様に機能します。これらの最適化されたオプティマイザーは、
...
% ここで、costFunctionは引数（ニューラル・ネットワーク・パラメーター）を
% 1つだけ取る関数です。
...
%% ================= パート 9: ウェイトを可視化 =================
%  隠れユニットがデータの中で何のフィーチャーを取り込んでいるかを確認するため、
%  隠れユニットを表示することにより、ニューラル・ネットワークが学習しているものを
%  「可視化」できるようになります。
...
%  トレーニング・セットのラベルを予測するために、ニューラルネット・ネットワークを
%  使用する「予測」機能を実装します。これにより、トレーニング・セットの精度を
%  計算できます。
