...
%COSTFUNCTIONREG 正則化されたロジスティック回帰のコストと勾配を計算する
%   J = COSTFUNCTIONREG(theta, X, y, lambda) は、正則化された
%   ロジスティック回帰のパラメーターとしてthetaを使用したコストと、
%   thetaを参照したコストの勾配を計算します。
...
%       Jにコストを設定する必要があります。偏微分を計算し、thetaの
%       各パラメーターを参照したコストの偏微分をgradに設定します。
%
