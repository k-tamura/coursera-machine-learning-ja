...
ロジスティック回帰のコスト（非正則化）は、<img src="https://latex.codecogs.com/gif.latex?\inline&space;j" title="j" />番目の要素が次のように定義されるベクトルであることを思い出してください。
...
画像のサイズは20×20なので、これは400の入力層単位を与えます（常に+1を出力する追加のバイアスユニットを除く）。 
